...
int main () {
    ...
    ll n, a, b, d;
    while (cin >> n >> a >> b >> d) {
        ll first = (fact[n] * mod_inverse(fact[a], MOD) % MOD * mod_inverse(fact[n-a], MOD) % MOD) % MOD;
        ll second = (fact[b] * mod_inverse(fact[d], MOD) % MOD * mod_inverse(fact[b-d], MOD) % MOD) % MOD;
        cout << (first * power(second , a, MOD)) % MOD << endl;
    }
    end();
    terminate;
}
